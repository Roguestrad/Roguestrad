...
void idAFConstraint_BallAndSocketJoint::SetPyramidLimit( const idVec3& pyramidAxis, const idVec3& baseAxis, const float angle1, const float angle2, const idVec3& body1Axis )
{
	if( coneLimit ) {
		delete coneLimit;
		coneLimit = NULL;
	...
	if( body2 ) {
		pyramidLimit->Setup(
			body1, body2, anchor2, pyramidAxis * body2->GetWorldAxis().Transpose(), baseAxis * body2->GetWorldAxis().Transpose(), angle1, angle2, body1Axis * body1->GetWorldAxis().Transpose() );
	} else {
		pyramidLimit->Setup( body1, body2, anchor2, pyramidAxis, baseAxis, angle1, angle2, body1Axis * body1->GetWorldAxis().Transpose() );
	}
}
...
void idAFConstraint_PyramidLimit::Setup(
		...
		if( body->children.Num() ) {
			for( j = 0; j < body->children.Num(); j++ ) {
				...
				if( !child->invI.InverseFastSelf() ) {
					gameLocal.Warning( "idAFTree::Factor: couldn't invert %dx%d matrix for constraint '%s'", child->invI.GetNumRows(), child->invI.GetNumColumns(), child->GetName().c_str() );
				}
			...
			if( !body->invI.InverseFastSelf() && child != NULL ) {
				gameLocal.Warning( "idAFTree::Factor: couldn't invert %dx%d matrix for body %s", child->invI.GetNumRows(), child->invI.GetNumColumns(), body->GetName().c_str() );
			}
...
void idPhysics_AF::CheckForCollisions( float timeStep )
{
	...
	for( i = 0; i < bodies.Num(); i++ ) {
		...
		if( body->clipMask != 0 ) {
			...
			if( gameLocal.clip.Motion( collision, body->current->worldOrigin, body->next->worldOrigin, rotation, body->clipModel, body->current->worldAxis, body->clipMask, passEntity ) ) {
...
bool idPhysics_AF::EvaluateContacts()
{
	...
	for( i = 0; i < bodies.Num(); i++ ) {
		...
		numContacts = gameLocal.clip.Contacts( contactInfo,
			10,
...
bool idPhysics_AF::Evaluate( int timeStepMSec, int endTimeMSec )
{
	...
	if( IsOutsideWorld() ) {
		gameLocal.Warning(
			"articulated figure moved outside world bounds for entity '%s' type '%s' at (%s)", self->name.c_str(), self->GetType()->classname, bodies[0]->current->worldOrigin.ToString( 0 ) );
		Rest();
	}
...
//!  Draws the silhouette edges of a clip model's trace model from a given projection origin.
...
void idPhysics_AF::DebugDraw()
{
	...
	if( af_highlightConstraint.GetString()[0] ) {
		...
		if( constraint ) {
			...
			if( af_showConstrainedBodies.GetBool() ) {
				...
				if( constrainedBody1 ) {
					collisionModelManager->DrawModel( constrainedBody1->clipModel->Handle(), constrainedBody1->clipModel->GetOrigin(), constrainedBody1->clipModel->GetAxis(), vec3_origin, 0.0f );
				}
				cvarSystem->SetCVarString( "cm_drawColor", colorBlue.ToString( 0 ) );
				constrainedBody2 = constraint->body2;
				if( constrainedBody2 ) {
					collisionModelManager->DrawModel( constrainedBody2->clipModel->Handle(), constrainedBody2->clipModel->GetOrigin(), constrainedBody2->clipModel->GetAxis(), vec3_origin, 0.0f );
				}
	...
	if( af_highlightBody.GetString()[0] ) {
		...
		if( highlightBody ) {
			...
			collisionModelManager->DrawModel( highlightBody->clipModel->Handle(), highlightBody->clipModel->GetOrigin(), highlightBody->clipModel->GetAxis(), vec3_origin, 0.0f );
			cvarSystem->SetCVarString( "cm_drawColor", colorRed.ToString( 0 ) );
		}
	}
	...
	if( af_showBodies.GetBool() ) {
		for( i = 0; i < bodies.Num(); i++ ) {
			...
			collisionModelManager->DrawModel( body->clipModel->Handle(), body->clipModel->GetOrigin(), body->clipModel->GetAxis(), vec3_origin, 0.0f );
	...
	if( af_showInertia.GetBool() ) {
		for( i = 0; i < bodies.Num(); i++ ) {
			...
			gameRenderWorld->DrawText( va( "\n\n\n( %.1f %.1f %.1f )\n( %.1f %.1f %.1f )\n( %.1f %.1f %.1f )", I[0].x, I[0].y, I[0].z, I[1].x, I[1].y, I[1].z, I[2].x, I[2].y, I[2].z ),
				body->GetWorldOrigin(),
				0.05f,
				colorCyan,
...
//! Saves the physics state of an articulated figure to a save file
...
//! Restores the physics state of an articulated figure from a save file.
...
void idPhysics_AF::BuildTrees()
{
	...
	if( af_useLinearTime.GetBool() ) {
		...
		if( trees.Num() > 1 ) {
			gameLocal.Warning( "Articulated figure has multiple separate tree structures for entity '%s' type '%s'.", self->name.c_str(), self->GetType()->classname );
		}
...
void idPhysics_AF::DeleteBody( const char* bodyName )
{
	...
	if( i >= bodies.Num() ) {
		gameLocal.Warning( "DeleteBody: no body found in the articulated figure with the name '%s' for entity '%s' type '%s'.", bodyName, self->name.c_str(), self->GetType()->classname );
		return;
	}
...
void idPhysics_AF::DeleteConstraint( const char* constraintName )
{
	...
	if( i >= constraints.Num() ) {
		gameLocal.Warning(
			"DeleteConstraint: no constriant found in the articulated figure with the name '%s' for entity '%s' type '%s'.", constraintName, self->name.c_str(), self->GetType()->classname );
		return;
	}
...
void idPhysics_AF::ClipTranslation( trace_t& results, const idVec3& translation, const idClipModel* model ) const
{
	...
	for( i = 0; i < bodies.Num(); i++ ) {
		...
		if( body->clipModel->IsTraceModel() ) {
			if( model ) {
				gameLocal.clip.TranslationModel( bodyResults,
					body->current->worldOrigin,
					body->current->worldOrigin + translation,
					body->clipModel,
					body->current->worldAxis,
				...
				gameLocal.clip.Translation( bodyResults, body->current->worldOrigin, body->current->worldOrigin + translation, body->clipModel, body->current->worldAxis, body->clipMask, self );
			}
		...
		if( body->clipModel->IsTraceModel() ) {
			if( model ) {
				gameLocal.clip.RotationModel(
					bodyResults, body->current->worldOrigin, rotation, body->clipModel, body->current->worldAxis, body->clipMask, model->Handle(), model->GetOrigin(), model->GetAxis() );
			} else {
				gameLocal.clip.Rotation( bodyResults, body->current->worldOrigin, rotation, body->clipModel, body->current->worldAxis, body->clipMask, self );
			}
			if( bodyResults.fraction < results.fraction ) {
				results = bodyResults;
			}
...
int idPhysics_AF::ClipContents( const idClipModel* model ) const
{
	...
	for( i = 0; i < bodies.Num(); i++ ) {
		...
		if( body->clipModel->IsTraceModel() ) {
			if( model ) {
				contents |= gameLocal.clip.ContentsModel( body->current->worldOrigin, body->clipModel, body->current->worldAxis, -1, model->Handle(), model->GetOrigin(), model->GetAxis() );
			} else {
