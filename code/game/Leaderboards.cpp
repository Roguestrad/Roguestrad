	...
	for( int mapIdx = 0; mapIdx < maps.Num(); mapIdx++ ) {
		for( int modeIdx = 0; modeIdx < numModes; modeIdx++ ) {
			...
			if( maps[mapIdx].supportedModes & BIT( modeIdx ) ) {
				...
				leaderboardDefinition_t* newLeaderboardDef =
					Sys_CreateLeaderboardDef( boardID, gamemode.numColumns, gamemode.columnDef, gamemode.rankOrder, gamemode.supportsAttachments, gamemode.checkAgainstCurrent );
