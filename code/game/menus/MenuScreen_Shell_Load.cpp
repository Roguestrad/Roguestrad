...
void idMenuScreen_Shell_Load::LoadGame( int index )
{
	...
	if( mgr != NULL && mgr->GetInGame() && !isDead ) {
		...
		if( index < sortedSaves.Num() ) {
			...
			common->Dialog().AddDialog( GDM_SP_LOAD_SAVE,
				DIALOG_ACCEPT_CANCEL,
				new idSWFScriptFunction_LoadDialog( GDM_SP_LOAD_SAVE, true, name.c_str() ),
				new idSWFScriptFunction_LoadDialog( GDM_SP_LOAD_SAVE, false, name.c_str() ),
				false );
		}
