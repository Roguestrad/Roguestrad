			...
			if( oldHealth > 0 ) {
				...
				if( player != NULL ) {
					...
					if( inflictor && inflictor->IsType( idMoveable::Type ) ) {
						...
						if( moveable != NULL && moveable->GetAttacker() != NULL && moveable->GetAttacker()->IsType( idPlayer::Type ) && moveable->GetAttacker() == attacker &&
							player->GetExpansionType() == GAME_D3XP && team != player->team ) {
