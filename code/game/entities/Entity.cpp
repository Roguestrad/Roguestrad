...
//! Updates GUI parameters from a dictionary of arguments.
...
//! Registers a user interface for rendering with the specified name and arguments.
			...
			if( spawnArgs.GetString( "bindToJoint", "", &joint ) && *joint ) {
				...
				if( ( parent->spawnArgs.GetString( "bindanim", "", &bindanim ) || parent->spawnArgs.GetString( "anim", "", &bindanim ) ) && *bindanim ) {
					...
					gameEdit->ANIM_CreateAnimFrame(
						parentAnimator->ModelHandle(), anim->MD5Anim( 0 ), parent->renderEntity.numJoints, frame, 0, parentAnimator->ModelDef()->GetVisualOffset(), parentAnimator->RemoveOrigin() );
