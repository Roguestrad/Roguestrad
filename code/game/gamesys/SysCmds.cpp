...
void Cmd_EntityList_f( const idCmdArgs& args )
{
	...
	for( e = 0; e < MAX_GENTITIES; e++ ) {
		...
		gameLocal.Printf( "%4i: %-20s %-20s %s\n", e, check->GetEntityDefName(), check->GetClassname(), check->name.c_str() );
...
static void Cmd_ShowViewNotes_f( const idCmdArgs& args )
{
	...
	if( parser.ExpectTokenString( "view" ) && parser.Parse1DMatrix( 3, origin.ToFloatPtr() ) && parser.Parse1DMatrix( 9, axis.ToFloatPtr() ) && parser.ExpectTokenString( "comments" ) &&
