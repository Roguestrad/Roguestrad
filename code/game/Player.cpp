			...
			if( !gameLocal.world->spawnArgs.GetBool( "no_Weapons" ) || ( weaponName == "weapon_fists" ) || ( weaponName == "weapon_soulcube" ) ) {
				if( ( weapons & ( 1 << i ) ) == 0 || common->IsMultiplayer() ) {
					...
					if( giveFlags & ITEM_GIVE_FEEDBACK ) {
						...
						if( lobby.GetLobbyUserWeaponAutoSwitch( lobbyUserID ) && idealWeapon != NULL && i != owner->weapon_bloodstone_active1 && i != owner->weapon_bloodstone_active2 &&
...
void idPlayer::RunPhysics_RemoteClientCorrection()
{
	...
	// Correction
	{
		...
		if( pm_clientAuthoritative_debug.GetBool() ) {
			// idLib::Printf( "[%d]Remote client player physics delta: %.2f. forward: %d pos <%.2f, %.2f, %.2f> \n", usercmd.clientGameFrame, delta, (int)usercmd.forwardmove, desiredPos.x,
			// desiredPos.y, desiredPos.z );
			gameRenderWorld->DebugLine( colorRed, newOrigin, desiredPos );
...
void idPlayer::UpdateHud()
{
	...
	if( gameLocal.realClientTime == lastMPAimTime ) {
		if( MPAim != -1 && gameLocal.mpGame.IsGametypeTeamBased() /* CTF */
			&& gameLocal.entities[MPAim] && gameLocal.entities[MPAim]->IsType( idPlayer::Type ) && static_cast<idPlayer*>( gameLocal.entities[MPAim] )->team == team ) {
...
void idPlayer::SetupHolsterSlot()
{
	...
	// can we holster?
	if( !( modelname = weapon->weaponDef->dict.GetString( "model" ) ) || strcmp( modelname, "models/weapons/soulcube/w_soulcube.lwo" ) == 0 || strcmp( modelname, "_DEFAULT" ) == 0 ||
		strcmp( modelname, "models/items/grenade_ammo/grenade.lwo" ) == 0 || !( renderModel = renderModelManager->FindModel( modelname ) ) ) {
	...
	} else if( strcmp( modelname, "models/weapons/shotgun/w_shotgun2.lwo" ) == 0 || strcmp( modelname, "models/weapons/bfg/bfg_world.lwo" ) == 0 ) {
...
void   idPlayer::UpdateLaserSight()
{
	...
	if( !IsGameStereoRendered() || !weapon.GetEntity()->ShowCrosshair() || AI_DEAD || weapon->IsHidden() || !weapon->GetMuzzlePosition( muzzleOrigin, muzzleAxis ) ) // Leyland: use GetMuzzlePosition
	{
		// hide it
...
void idPlayer::RemoveAllButEssentialWeapons()
{
	...
	for( ; kv != NULL; kv = spawnArgs.MatchPrefix( "def_weapon", kv ) ) {
		...
		if( kv->GetValue() == "weapon_fists" || kv->GetValue() == "weapon_soulcube" || kv->GetValue() == "weapon_pda" || kv->GetValue() == "weapon_flashlight" ||
