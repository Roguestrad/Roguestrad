...
void idStaticEntity::Spawn()
{
	...
	if( gameLocal.world->spawnArgs.GetBool( "inlineAllStatics" ) ) {
		...
		bool isGUI = false;
		if( spawnArgs.GetString( "gui", NULL ) != NULL || spawnArgs.GetString( "gui2", NULL ) != NULL || spawnArgs.GetString( "gui3", NULL ) != NULL ) {
			isGUI = true;
		}
		...
		if( ( extension.Icmp( GLTF_GLB_EXT ) == 0 ) || ( extension.Icmp( GLTF_EXT ) == 0 ) || ( extension.Icmp( "lwo" ) == 0 ) || ( extension.Icmp( "ase" ) == 0 ) || ( extension.Icmp( "ma" ) == 0 ) ||
...
{
	if( ( renderEntity.shaderParms[SHADERPARM_BEAM_END_X] != origin.x ) || ( renderEntity.shaderParms[SHADERPARM_BEAM_END_Y] != origin.y ) ||
...
void idPhantomObjects::Think()
{
	...
	for( i = 0; i < targets.Num(); i++ ) {
		...
		if( time < 0.0f ) {
			idAI::PredictTrajectory( entPhys->GetOrigin(),
				lastTargetPos[i],
				speed,
				entPhys->GetGravity(),
