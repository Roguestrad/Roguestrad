...
const char*	  idMultiplayerGame::skinNames[]  = { "skins/characters/player/marine_mp",
...
// handy verbose
const char* idMultiplayerGame::GameStateStrings[] = { "INACTIVE", "WARMUP", "COUNTDOWN", "GAMEON", "SUDDENDEATH", "GAMEREVIEW", "NEXTGAME" };
...
void idMultiplayerGame::GetSpectateText( idPlayer* player, idStr spectatetext[2], bool scoreboard )
{
	...
	if( player->wantSpectate && !scoreboard ) {
		if( gameLocal.gameType == GAME_LASTMAN && gameState == GAMEON ) {
			...
			if( gameLocal.gameType == GAME_TOURNEY && ( currentTourneyPlayer[0] != -1 && currentTourneyPlayer[1] != -1 ) &&
	...
	switch( evt ) {
		...
		case MSG_JOINTEAM:
			AddChatLine( idLocalization::GetString( "#str_04280" ),
				...
				parm2 ? idLocalization::GetString( "#str_02500" ) : idLocalization::GetString( "#str_02499" ) );
			break;
		case MSG_HOLYSHIT:
