...
{
	origin.Set( m_ScaleX * poseMat.m[2][3], m_ScaleY * poseMat.m[0][3], m_ScaleZ * poseMat.m[1][3] );
...
bool VRSystem_Valve::InitHMD()
{
	...
	hmd->GetProjectionRaw( vr::Eye_Right, &openVRfovEye[1][0], &openVRfovEye[1][1], &openVRfovEye[1][2], &openVRfovEye[1][3] );

	hmd->GetProjectionRaw( vr::Eye_Left, &openVRfovEye[0][0], &openVRfovEye[0][1], &openVRfovEye[0][2], &openVRfovEye[0][3] );

	openVRScreenSeparation = 0.5f * ( openVRfovEye[1][1] + openVRfovEye[1][0] ) / ( openVRfovEye[1][1] - openVRfovEye[1][0] ) -
							 0.5f * ( openVRfovEye[0][1] + openVRfovEye[0][0] ) / ( openVRfovEye[0][1] - openVRfovEye[0][0] );
...
void VRSystem_Valve::GenMouseEvents()
{
	...
	if( m_HadHead && tr.guiModel->GetVRShell( shellOrigin, shellAxis ) ) {
		...
		idVec3		upperLeft	  = shellOrigin + shellAxis[0] * guiForward + shellAxis[1] * 0.5f * guiWidth + shellAxis[2] * 0.5f * guiHeight;
		...
		if( rayDir.x != 0 ) {
			...
			if( x >= 0 && x < renderSystem->GetWidth() && y >= 0 && y < renderSystem->GetHeight() && ( x != oldX || y != oldY ) ) {
				oldX = x;
				oldY = y;
...
bool		  VRSystem_Valve::GetLeftControllerAxis( idVec2& axis )
{
	...
	if( openVRLeftTouchpad ) {
		...
		if( fabs( m_LeftControllerState.rAxis[0].x ) < threshold && fabs( m_LeftControllerState.rAxis[0].y ) < threshold ) {
			return false;
		}
	}
...
bool VRSystem_Valve::GetRightControllerAxis( idVec2& axis )
{
	...
	if( openVRRightTouchpad ) {
		...
		const float threshold = joy_deadZone.GetFloat();
		if( fabs( m_RightControllerState.rAxis[0].x ) < threshold && fabs( m_RightControllerState.rAxis[0].y ) < threshold ) {
			return false;
		}
	}
