...
#define CONSOLE_COMMAND_NO_COMPILE( name, comment, completion ) static inline void name##_f( const idCmdArgs& args )

// lint incorrectly gives this for all console commands: Issue 1568: (Warning -- Variable 'TestAtomicString_v' accesses variable 'atomicStringManager' before the latter is initialized through calls:
// 'TestAtomicString_f() => idAtomicString::FreeDynamic()') I can't figure out how to disable this just around CONSOLE_COMMAND, so it must stay disabled everywhere, which is a shame.
// lint -e1568
