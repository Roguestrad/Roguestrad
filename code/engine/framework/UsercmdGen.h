...
public:
	...
	bool HasUserCmdForClientTimeBuffer( int playerIndex, int millisecondBuffer )
	{
		...
		if( millisecondBuffer == 0 ) { return HasUserCmdForPlayer( playerIndex ); }

		if( GetNumUnreadFrames( playerIndex ) < 2 ) { return false; }
	...
	const usercmd_t& GetUserCmdForPlayer( int playerIndex )
	{
		...
		if( readFrame[playerIndex] < writeFrame[playerIndex] - 1 ) { readFrame[playerIndex]++; }
