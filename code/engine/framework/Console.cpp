...
float idConsoleLocal::DrawFPS( float y )
{
	...
	const uint64 rendererGPUShaderPassesTime = commonLocal.GetRendererGpuShaderPassMicroseconds() + commonLocal.GetRendererGpuFogAllLightsMicroseconds() +
											   commonLocal.GetRendererGpuShaderPassPostMicroseconds() + commonLocal.GetRendererGpuDrawGuiMicroseconds();
	...
	const int64 rendererCPUBusyTime		= int64( commonLocal.mainFrameTiming.finishSyncTime_EndFrame - commonLocal.mainFrameTiming.startGameTime ) +
									  Min( Max( int64( 0 ), rendererMvkEncodeTime - rendererQueueSubmitTime - gameThreadWaitTime ), frameSyncTime - rendererQueueSubmitTime );
#else
	const int64 rendererCPUBusyTime = int64( commonLocal.mainFrameTiming.finishSyncTime_EndFrame - commonLocal.mainFrameTiming.startGameTime );
#endif
	...
	if( ImGuiHook::IsReadyToRender() ) {
		...
		ImGui::TextColored( colorLtGrey,
			"GENERAL: views:%i draws:%i tris:%i",
		...
		if( com_showFPS.GetInteger() > 2 ) {
			...
			ImGui::TextColored( colorLtGrey,
				"SHADOWS: atlas usage:%2i%% views:%i draws:%i tris:%i",
				atlasPercentage,
			...
			ImGui::TextColored( colorLtGrey,
				"DYNAMIC: callback:%-2i md5:%i dfrmVerts:%i dfrmTris:%i tangTris:%i guis:%i",
			...
			ImGui::TextColored( colorLtGrey,
				"MASKCULL: tests:%-3i lightCulls:%i surfCulls:%i verts:%i tris:%i",
			...
			ImGui::TextColored( colorLtGrey,
				"ADDMODEL: callback:%-2i createInteractions:%i createShadowVolumes:%i",
				commonLocal.stats_frontend.c_entityDefCallbacks,
			...
			ImGui::TextColored( colorLtGrey,
				"UPDATES: entityUpdates:%-3i  entityRefs:%-3i  lightUpdates:%-2i  lightRefs:%i\n",
		...
		ImGui::TextColored(
			rendererMaskedOcclusionCullingTime > maxTime ? colorRed : colorWhite, "MOC:     %5llu us   Shadow Atlas: %5llu us", rendererMaskedOcclusionCullingTime, rendererGPUShadowAtlasTime );
#if defined( __APPLE__ ) && defined( USE_MoltenVK )
		...
		ImGui::TextColored( rendererMvkEncodeTime > maxTime || rendererGPUInteractionsTime > maxTime ? colorRed : colorWhite,
			"Encode:  %5lld us   Interactions: %5llu us",
			rendererMvkEncodeTime,
			rendererGPUInteractionsTime );
	...
	if( consoleField.GetAutoCompleteLength() != 0 ) {
		...
		if( autoCompleteLength > 0 ) {
			renderSystem->DrawFilled( idVec4( 0.8f, 0.2f, 0.2f, 0.45f ),
				...
				y + 2,
				autoCompleteLength * SMALLCHAR_WIDTH,
				SMALLCHAR_HEIGHT - 2 );
		}
	}
...
#define VERSION_LINE_SPACE ( SMALLCHAR_HEIGHT + 4 )
	...
	for( x = 0; x < i; x++ ) {
		renderSystem->DrawSmallChar( LOCALSAFE_WIDTH - ( i - x ) * SMALLCHAR_WIDTH, ( lines - ( SMALLCHAR_HEIGHT + SMALLCHAR_HEIGHT / 4 ) ) - VERSION_LINE_SPACE - VERSION_LINE_SPACE, version[x] );
	}
	// jmarshall
	...
	for( x = 0; x < i; x++ ) {
		renderSystem->DrawSmallChar( LOCALSAFE_WIDTH - ( i - x ) * SMALLCHAR_WIDTH, ( lines - ( SMALLCHAR_HEIGHT + SMALLCHAR_HEIGHT / 2 ) ) - ( VERSION_LINE_SPACE - 2 ), branchVersion[x] );
	}
	...
	for( x = 0; x < i; x++ ) {
		renderSystem->DrawSmallChar( LOCALSAFE_WIDTH - ( i - x ) * SMALLCHAR_WIDTH, ( lines - ( SMALLCHAR_HEIGHT + SMALLCHAR_HEIGHT / 2 ) ), builddate[x] );
	}
	// jmarshall end
