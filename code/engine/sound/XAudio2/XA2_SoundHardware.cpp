...
std::vector<AudioDevice> EnumerateAudioDevices( _Out_opt_ AudioDevice* defaultDevice = nullptr )
{
	...
	HRESULT					 hResult = CoCreateInstance( __uuidof( MMDeviceEnumerator ), NULL, CLSCTX_ALL, __uuidof( IMMDeviceEnumerator ), ( void** )&immDevEnum );
...
// RB: not available on Windows 8 SDK
	...
	if( !vAudioDevices.empty() ) {
		...
		if( SUCCEEDED( pXAudio2->CreateMasteringVoice( &pMasterVoice, XAUDIO2_DEFAULT_CHANNELS, outputSampleRate, 0, selectedDevice.id.c_str(), NULL, AudioCategory_GameEffects ) ) ) {
...
void idSoundHardware_XAudio2::Shutdown()
{
	...
	if( pXAudio2 != NULL ) {
		...
		// idLib::Printf( "Final pXAudio2 performanceData: Voices: %d/%d CPU: %.2f%% Mem: %dkb\n", perfData.ActiveSourceVoiceCount, perfData.TotalSourceVoiceCount, perfData.AudioCyclesSinceLastQuery /
		// ( float )perfData.TotalCyclesSinceLastQuery, perfData.MemoryUsageInBytes / 1024 );
		pXAudio2->Release();
		pXAudio2 = NULL;
	}
	if( vuMeterRMS != NULL ) {
...
void idSoundHardware_XAudio2::Update()
{
	...
	if( s_showPerfData.GetBool() ) {
		...
		idLib::Printf( "Voices: %d/%d CPU: %.2f%% Mem: %dkb\n",
			...
			perfData.AudioCyclesSinceLastQuery / ( float )perfData.TotalCyclesSinceLastQuery,
