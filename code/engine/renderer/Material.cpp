...
void idMaterial::MultiplyTextureMatrix( textureStage_t* ts, int registers[2][3] )
{
	...
	ts->matrix[0][0] = EmitOp( EmitOp( old[0][0], registers[0][0], OP_TYPE_MULTIPLY ), EmitOp( old[0][1], registers[1][0], OP_TYPE_MULTIPLY ), OP_TYPE_ADD );
	ts->matrix[0][1] = EmitOp( EmitOp( old[0][0], registers[0][1], OP_TYPE_MULTIPLY ), EmitOp( old[0][1], registers[1][1], OP_TYPE_MULTIPLY ), OP_TYPE_ADD );
	ts->matrix[0][2] = EmitOp( EmitOp( EmitOp( old[0][0], registers[0][2], OP_TYPE_MULTIPLY ), EmitOp( old[0][1], registers[1][2], OP_TYPE_MULTIPLY ), OP_TYPE_ADD ), old[0][2], OP_TYPE_ADD );

	ts->matrix[1][0] = EmitOp( EmitOp( old[1][0], registers[0][0], OP_TYPE_MULTIPLY ), EmitOp( old[1][1], registers[1][0], OP_TYPE_MULTIPLY ), OP_TYPE_ADD );
	ts->matrix[1][1] = EmitOp( EmitOp( old[1][0], registers[0][1], OP_TYPE_MULTIPLY ), EmitOp( old[1][1], registers[1][1], OP_TYPE_MULTIPLY ), OP_TYPE_ADD );
...
void idMaterial::ParseStage( idLexer& src, const textureRepeat_t trpDefault )
{
	...
	while( 1 ) {
		...
		if( !token.Icmp( "rotate" ) ) {
			...
			matrix[0][2] = EmitOp( EmitOp( EmitOp( GetExpressionConstant( -0.5 ), cosReg, OP_TYPE_MULTIPLY ), EmitOp( GetExpressionConstant( 0.5 ), sinReg, OP_TYPE_MULTIPLY ), OP_TYPE_ADD ),
				GetExpressionConstant( 0.5 ),
				OP_TYPE_ADD );

			matrix[1][0] = sinReg;
			matrix[1][1] = cosReg;
			matrix[1][2] = EmitOp( EmitOp( EmitOp( GetExpressionConstant( -0.5 ), sinReg, OP_TYPE_MULTIPLY ), EmitOp( GetExpressionConstant( -0.5 ), cosReg, OP_TYPE_MULTIPLY ), OP_TYPE_ADD ),
				GetExpressionConstant( 0.5 ),
				OP_TYPE_ADD );
...
bool idMaterial::Parse( const char* text, const int textLength, bool allowBinaryVersion )
{
	...
	if( coverage == MC_BAD ) {
		...
		if( !numStages ) {
			...
			coverage = MC_OPAQUE;
		} else if( ( pd->parseStages[0].drawStateBits & GLS_DSTBLEND_BITS ) != GLS_DSTBLEND_ZERO || ( pd->parseStages[0].drawStateBits & GLS_SRCBLEND_BITS ) == GLS_SRCBLEND_DST_COLOR ||
				   ( pd->parseStages[0].drawStateBits & GLS_SRCBLEND_BITS ) == GLS_SRCBLEND_ONE_MINUS_DST_COLOR || ( pd->parseStages[0].drawStateBits & GLS_SRCBLEND_BITS ) == GLS_SRCBLEND_DST_ALPHA ||
...
===================
*/
const char* opNames[] = { "OP_TYPE_ADD",
	"OP_TYPE_SUBTRACT",
	"OP_TYPE_MULTIPLY",
	"OP_TYPE_DIVIDE",
	"OP_TYPE_MOD",
...
CONSOLE_COMMAND_SHIP( makeMaterials, "Make .mtr file from a models or textures folder using PBR conventions", idCmdSystem::ArgCompletion_ImageName )
{
	...
	for( int i = 0; i < files->GetNumFiles(); i++ ) {
		...
		if( idStr::FindText( imageName, "_diffuse", false ) != -1 || idStr::FindText( imageName, "_diff", false ) != -1 || idStr::FindText( imageName, "_albedo", false ) != -1 ||
			idStr::FindText( imageName, "_basecolor", false ) != -1 || idStr::FindText( imageName, "_base", false ) != -1 || idStr::FindText( imageName, "_color", false ) != -1 ||
			...
			if( idStr::FindText( baseName, "_inst_inst_inst_", false ) != -1 || idStr::FindText( baseName, "_inst_inst_", false ) != -1 || idStr::FindText( baseName, "_inst_", false ) != -1 ) {
