...
void idRenderModelMD3::InitFromFile( const char* fileName, const idImportOptions* options )
{
	...
	if( version != MD3_VERSION ) {
		...
		common->Warning( "InitFromFile: %s has wrong version (%i should be %i)", fileName, version, MD3_VERSION );
		return;
	}
	...
	for( i = 0; i < md3->numSurfaces; i++ ) {
		...
		if( surf->numVerts > SHADER_MAX_VERTEXES ) {
			common->Error( "InitFromFile: %s has more than %i verts on a surface (%i)", fileName, SHADER_MAX_VERTEXES, surf->numVerts );
		}
		if( surf->numTriangles * 3 > SHADER_MAX_INDEXES ) {
			common->Error( "InitFromFile: %s has more than %i triangles on a surface (%i)", fileName, SHADER_MAX_INDEXES / 3, surf->numTriangles );
		}
