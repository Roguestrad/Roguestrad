...
void idSWFSprite::WriteSVGUnfolded_r( idFile* f, int characterID, const idList< idSWFDictionaryEntry, TAG_SWF >& dict,
									  const swfMatrix_t& parentMatrix, const swfColorXform_t& parentColor, int indent )
{
	idStr tabs;
	tabs.Fill( '\t', indent );
	...
	for( int i = 0; i < commands.Num(); i++ )
	{
		...
		switch( command.tag )
		{
#define HANDLE_SWF_TAG( x ) case Tag_##x: WriteSVGUnfolded_##x( f, command.stream, characterID, i, dict, parentMatrix, parentColor, indent + 1 ); break;
				HANDLE_SWF_TAG( PlaceObject2 );
				//HANDLE_SWF_TAG( PlaceObject3 );
				//HANDLE_SWF_TAG( RemoveObject2 );
				//HANDLE_SWF_TAG( StartSound );
...
void idSWFSprite::WriteSVGUnfolded_PlaceObject2( idFile* file, idSWFBitStream& bitstream, int sourceCharacterID, int commandID, const idList< idSWFDictionaryEntry, TAG_SWF >& dict,
		const swfMatrix_t& parentMatrix, const swfColorXform_t& parentColor, int indent )
{
	uint8 flags1 = bitstream.ReadU8();
	int depth = bitstream.ReadU16();
