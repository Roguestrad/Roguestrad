...
int idJoystickWin32::PollInputEvents( int inputDeviceNum )
{
	...
	for( int i = 0; i < 32; i++ ) {
		...
		if( ( ( xis.Gamepad.wButtons | old.Gamepad.wButtons ) & bit ) == 0 && ( orBits & bit ) ) {
	...
	int joyRemap[16] = {
		...
		J_DPAD_DOWN, // Up, Down
		J_DPAD_LEFT,
		J_DPAD_RIGHT, // Left, Right
		J_ACTION9,
		J_ACTION10, // Start, Back
		J_ACTION7,
		J_ACTION8, // Left Stick Down, Right Stick Down
		J_ACTION5,
		J_ACTION6, // Black, White (Left Shoulder, Right Shoulder)
		0,
		0, // Unused
		J_ACTION1,
		J_ACTION2, // A, B
