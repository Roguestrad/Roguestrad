...
idCVar		  net_peerTimeoutInSeconds(
	   "net_peerTimeoutInSeconds", "30", CVAR_INTEGER, "If the host hasn't received a response from a peer in this amount of time (in seconds), the peer will be disconnected." );
idCVar net_peerTimeoutInSeconds_Lobby(
	"net_peerTimeoutInSeconds_Lobby", "20", CVAR_INTEGER, "If the host hasn't received a response from a peer in this amount of time (in seconds), the peer will be disconnected." );
...
idCVar net_bw_test_throttle_rate_pct(
	"net_bw_test_throttle_rate_pct", "0.80", CVAR_FLOAT, "Min rate % a peer must match in bandwidth challenge before being throttled. 1.0=perfect, 0.0=received nothing" );
idCVar net_bw_test_throttle_byte_pct(
	"net_bw_test_throttle_byte_pct", "0.80", CVAR_FLOAT, "Min byte % a peer must match in bandwidth challenge before being throttled. 1.0=perfect (received everything) 0.0=Received nothing" );
idCVar				net_bw_test_throttle_seq_pct( "net_bw_test_throttle_seq_pct",
	 "0.80",
	 CVAR_FLOAT,
...
void		  idLobby::DrawDebugNetworkHUD() const
{
	...
	renderSystem->DrawSmallStringExt(
		idMath::Ftoi( X_OFFSET ), idMath::Ftoi( curY ), "# Peer                   | Sent kB/s | Recv kB/s | Sent MB | Recv MB | Ping   | L |  %  | R.NM | R.SZ | R.AK | T", colorGreen, false );
	curY += Y_SPACING;

	renderSystem->DrawSmallStringExt( idMath::Ftoi( X_OFFSET ),
		idMath::Ftoi( curY ),
		"------------------------------------------------------------------------------------------------------------------------------------",
		colorGreen,
		false );
	...
	for( int p = 0; p < peers.Num(); p++ ) {
		...
		renderSystem->DrawSmallStringExt( X_OFFSET,
			curY,
			va( "%i %22s | %2.02f kB/s | %2.02f kB/s | %2.02f MB | %2.02f MB |%4i ms | %i | %i%% | %i | %i | %i | %2.2f / %2.2f / %i",
				p,
				name.c_str(),
				sentKps,
				recvKps,
				...
				peer.loaded,
				resourcePercent,
				peer.packetProc->NumQueuedReliables(),
	...
	if( net_forceUpstream.GetFloat() != 0.0f ) {
		...
		renderSystem->DrawSmallStringExt( X_OFFSET,
			curY,
			va( "Queued: %d | Dropping: %2.02f kB/s Queue: %2.02f kB/s -> Effective %2.02f kB/s",
				queuedBytes,
				upstreamDropRate / 1024.0f,
...
void idLobby::DrawDebugNetworkHUD2() const
{
	...
	for( int p = 0; p < peers.Num(); p++ ) {
		...
		renderSystem->DrawSmallStringExt( X_OFFSET,
			curY,
			va( "%i - %s | %2.02f kB/s | %2.02f kB/s | %i | %i | %d/%d", p, peerName.c_str(), sentKps, recvKps, peers[p].loaded, peers[p].address.UsingRelay(), rLoaded, rTotal ),
			color,
			false );
		curY += Y_SPACING;
	}
...
void   idLobby::DrawDebugNetworkHUD_ServerSnapshotMetrics( bool draw )
{
	...
	for( int p = 0; p < peers.Num(); p++ ) {
		...
		enum netDebugGraphs_t { GRAPH_SNAPSENT, GRAPH_OUTGOING, GRAPH_INCOMINGREPORTED, GRAPH_MAX };
		...
		renderSystem->DrawSmallStringExt( X_OFFSET,
			curY,
			va( "Peer %d - %s RTT %d %sPeerSnapRate: %d %s", p, GetPeerName( p ), peer.lastPingRtt, throttled ? "^1" : "^2", peer.throttledSnapRate / 1000, throttled ? "^1Throttled" : "" ),
			color,
			false );
		curY += Y_SPACING;
		...
		renderSystem->DrawSmallStringExt(
			X_OFFSET, curY, va( "Reliables: %d / %d bytes Reliable Ack: %d", packetProc->NumQueuedReliables(), packetProc->GetReliableDataSize(), packetProc->NeedToSendReliableAck() ), color, false );
		curY += Y_SPACING;

		renderSystem->DrawSmallStringExt( X_OFFSET,
			curY,
			va( "Outgoing %.2f kB/s  Reported %.2f kB/s Throttle: %.2f", peer.packetProc->GetOutgoingRateBytes() / 1024.0f, peers[p].receivedBps / 1024.0f, peer.receivedThrottle ),
			color,
			false );
		curY += Y_SPACING;
		...
		if( net_forceUpstream.GetFloat() != 0.0f ) {
			...
			int	  queuedBytes		 = session->GetQueuedBytes();
			renderSystem->DrawSmallStringExt(
				X_OFFSET, curY, va( "Queued: %d | Dropping: %2.02f kB/s Queue: %2.02f kB/s ", queuedBytes, upstreamDropRate / 1024.0f, upstreamQueuedRate / 1024.0f ), color, false );
		}
...
void idLobby::ServerUpdateBandwidthTest()
{
	...
	for( int i = 0; i < peers.Num(); ++i ) {
		...
		unsigned int randomSize =
			Min( ( unsigned int )( sizeof( buffer ) - 12 ), ( unsigned int )session->GetTitleStorageInt( "net_bw_test_packetSizeBytes", net_bw_test_packetSizeBytes.GetInteger() ) );
