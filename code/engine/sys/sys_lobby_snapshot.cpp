...
idCVar net_peer_throttle_bps_peer_threshold_pct(
	"net_peer_throttle_bps_peer_threshold_pct", "0.60", CVAR_FLOAT, "Min reported incoming bps % of sent from host that a peer must maintain before throttling kicks in" );
	...
	if( size != 0 ) {
		if( size > 0 ) {
			NET_VERBOSESNAPSHOT_PRINT_LEVEL( 3,
				va( "NET: (peer %d) Sending snapshot %d delta'd against %d. Since JobSub: %d Since LastSend: %d. Size: %d\n",
					p,
					peer.snapProc->GetSnapSequence(),
					peer.snapProc->GetBaseSequence(),
					timeFromJobSub,
					timeFromLastSend,
					size ) );
	...
	if( peer_throttle_bps_host_threshold > 0.0f ) {
		...
		if( deltaT > 0 && peer.receivedThrottleTime > 0 && peer.receivedBpsIndex > 0 ) {
			...
			if( sentBps > peer_throttle_bps_host_threshold ) {
				float pct =
					peer.receivedBps / idMath::ClampFloat( 0.01f, static_cast<float>( BANDWIDTH_REPORTING_MAX ), sentBps ); // note the receivedBps is implicitly clamped on client end to 10k/sec
