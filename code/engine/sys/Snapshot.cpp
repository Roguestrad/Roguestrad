...
idCVar net_verboseSnapshotCompression(
	"net_verboseSnapshotCompression", "0", CVAR_INTEGER | CVAR_NOCHEAT, "Verbose snapshot code to help debug snapshot problems. Greater the number greater the spam" );
			...
			if( state.buffer.Size() == 0 ) {
				...
				if( objTemplateState != NULL && objTemplateState->buffer.Size() > 0 && sequence >= objTemplateState->expectedSequence ) {
					idLib::PrintfIf( net_ssTemplateDebug.GetBool(),
						...
						sequence,
						baseSequence,
						objTemplateState->expectedSequence,
						deltaSize );
