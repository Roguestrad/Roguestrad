...
/*!
	\class idBTreeNode
	\brief B+ tree node implementation for efficient data storage and retrieval.
*/
...
/*!
	\class idBTree
	\brief A B-tree implementation for efficient key-based object storage and retrieval.

	This class provides a templated B-tree data structure that organizes objects based on keys for fast lookup, insertion, and deletion operations. The tree maintains balance through node splitting
   and merging operations to ensure efficient performance. It supports various search methods including exact match, ceiling, and floor lookups. The implementation handles memory management through
   dedicated node allocation and deallocation functions. The tree structure is designed to be self-balancing, with internal methods ensuring proper node splitting when capacity is exceeded and merging
   when nodes become too empty. The class is intended for use in scenarios requiring ordered storage with efficient key-based access patterns.
...
{
public:
	//! Constructs an empty B-tree with the specified template parameters.
	idBTree();

	//! Destroys the b-tree and releases its resources.
	~idBTree();

	//! Initializes the B-tree by allocating a root node
	void						   Init();

	//! Destroys all nodes in the B-tree and resets the root pointer
	void						   Shutdown();

	//! Adds an object to the B-tree with the specified key and returns a pointer to the new tree node.
	idBTreeNode<objType, keyType>* Add( objType* object, keyType key );

	//! Removes an object node from the tree.
	void						   Remove( idBTreeNode<objType, keyType>* node );

	//! Finds and returns a node in the B-tree that matches the given key.
	idBTreeNode<objType, keyType>* NodeFind( keyType key ) const;

	//! Finds the node with the smallest key larger than or equal to the given key.
	idBTreeNode<objType, keyType>* NodeFindSmallestLargerEqual( keyType key ) const;

	//! Finds the node with the largest key smaller than or equal to the given key.
	...
	//! Returns the next node in the tree traversal order
	idBTreeNode<objType, keyType>* GetNext( idBTreeNode<objType, keyType>* node ) const;

	//! Returns the next leaf node in the tree traversal order.
...
private:
	...
	//! Allocates and initializes a new B-tree node with default values.
	idBTreeNode<objType, keyType>*					 AllocNode();

	//! Frees the memory of the given B-tree node using the node allocator
	void											 FreeNode( idBTreeNode<objType, keyType>* node );

	//! Splits a B-tree node into two nodes when the node exceeds the maximum number of children.
	void											 SplitNode( idBTreeNode<objType, keyType>* node );

	//! Merges two B-tree nodes into a single node.
	idBTreeNode<objType, keyType>*					 MergeNodes( idBTreeNode<objType, keyType>* node1, idBTreeNode<objType, keyType>* node2 );

	//! Checks the validity of a B-tree node and its children during tree traversal.
	void											 CheckTree_r( idBTreeNode<objType, keyType>* node, int& numNodes ) const;

	//! Validates the integrity of the B-tree structure by checking node counts and key ordering.
