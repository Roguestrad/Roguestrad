...
/*!
	...
	\brief idSurface_SweptSpline represents a surface generated by sweeping one spline along another.

	This class implements a surface that is created by sweeping a spline geometry along a path defined by another spline. It provides functionality for setting the base spline and the swept spline,
   initializing swept shapes like circles, and tessellating the resulting surface. The class manages its own spline data and ensures proper cleanup of resources. The tessellation process generates
   vertex data and index information for rendering the swept surface. The GetFrame method enables computation of coordinate frames along the swept path, which is useful for maintaining proper
   orientation during the sweeping operation. The class inherits from idSurface, indicating it is designed to be used as a surface geometry within the engine's rendering system.
