...
void gltfItem_mat4::parse( idToken& token )
{
	...
	double* val = numbers->item->Ptr();
	*item		= idMat4( val[0], val[1], val[2], val[3], val[4], val[5], val[6], val[7], val[8], val[9], val[10], val[11], val[12], val[13], val[14], val[15] );
	delete numbers;
}
...
void gltfItem_KHR_lights_punctual::parse( idToken& token )
{
	...
	for( auto& prop : array ) {
		...
		item->KHR_lights_punctual.AssureSizeAlloc( item->KHR_lights_punctual.Num() + 1, idListNewElement<gltfExt_KHR_lights_punctual> );

		gltfExt_KHR_lights_punctual* gltfLight = item->KHR_lights_punctual[item->KHR_lights_punctual.Num() - 1];
...
idList<idMat4>& gltfData::GetAccessorViewMat( gltfAccessor* accessor )
{
	...
	if( matView == nullptr ) {
		...
		idFile_Memory bin = idFile_Memory( "GetAccessorView(idMat4*)", ( const char* )( ( attrData->GetData( attrBv->buffer ) + attrBv->byteOffset + accessor->byteOffset ) ), attrBv->byteLength );
